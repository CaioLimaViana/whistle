function request(url,args,mode)
{
    if(mode === "headless") fetch(url,args);
    else if(mode === "new_window") window.open(url,"_blank");
    else if(mode === "new_window_close")
    {
        let window_identifier = window.open(url,"_blank");
        setTimeout(()=>{
            if(window_identifier) window_identifier.close();
        },100);
    }
}
